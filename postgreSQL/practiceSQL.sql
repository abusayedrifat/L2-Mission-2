...
--2
SELECT department_name, round(avg(salary)),hire_date as avg_salaryByDept FROM employees
JOIN departments USING(department_id)  GROUP BY department_name;

--> 2-note: when we use  GROUP BY we can SELECT only that column or such thing that are used in GROUP BY clause and we can use aggregate function 

--3
SELECT department_name, count(department_name) as employee_per_Dept FROM employees
JOIN departments USING(department_id) GROUP BY department_name ;
